    ...
    with tab1:
        ...
        if st.button("Jalankan Model Regresi"):
            with st.spinner("Sedang melatih pohon keputusan..."):
                ...
                # Plot learning curve untuk regresi
                train_sizes, train_scores, val_scores = learning_curve(
                    rf_reg, X_train_reg, y_train_reg,
                    cv=5, scoring='r2',
                ...
                plt.xlabel("Jumlah Contoh Latih")
                plt.ylabel("Skor (R²)")
                plt.legend(loc="best")
                st.pyplot(fig)
    ...
    with tab2:
        ...
        if st.button("Jalankan Model Klasifikasi"):
            with st.spinner("Sedang mengklasifikasikan risiko..."):
                ...
                # Plot learning curve untuk klasifikasi
                train_sizes, train_scores, val_scores = learning_curve(
                    rf_cls, X_train_cls, y_train_cls,
                ...
                plt.title("Learning Curve (Klasifikasi)")
                plt.xlabel("Jumlah Contoh Latih")
                plt.ylabel("Akurasi")
                plt.legend(loc="best")
                st.pyplot(fig)